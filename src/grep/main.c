...
int add_pattern_to_patterns(keys_t* patterns, const char* str) {
  ...
  if (patterns->data_size == patterns->alloc_size) {
    ...
    char** new_data =
        (char**)safe_alloc(patterns->data, new_alloc_size, sizeof(char*), &rc);
    if (!rc) {
...
int main(int argc, char** argv) {
  ...
  if (!exit_code) {
    while (argind < argc) {
      exit_code = read_file_to_buffer(argv[argind], &inbuf);
      // TODO: grep
      if (!exit_code) {
        printf("1\n");
  ...
  if (patterns.data) {
    for (int i = 0; i < patterns.data_size; i++) {
      printf("%s\n", patterns.data[i]);
...
//                 qsort(matches, match_count, sizeof(regmatch_t),
//                 compare_matches); if (!opts.out_invert) {
